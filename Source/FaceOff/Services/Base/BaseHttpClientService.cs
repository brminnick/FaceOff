...
namespace FaceOff
{
	abstract class BaseHttpClientService
	{
		...
		protected static Task<HttpResponseMessage> PatchObjectToAPI<T>(string apiUrl, T requestData) =>
			SendAsync(new HttpMethod("PATCH"), apiUrl, requestData);
		...
		static HttpClient CreateHttpClient(TimeSpan timeout)
		{
			...
			client.DefaultRequestHeaders.Add("ocp-apim-subscription-key", CognitiveServicesConstants.FaceApiKey);
		...
		static async ValueTask<HttpRequestMessage> GetHttpRequestMessage<T>(HttpMethod method, string apiUrl, T requestData = default)
		{
			...
			switch (requestData)
			{
				...
				case Stream stream:
					httpRequestMessage.Content = new StreamContent(requestData as Stream);
		...
		static void Report(Exception e, [CallerMemberName]string callerMemberName = "") => AnalyticsHelpers.Report(e, callerMemberName: callerMemberName);
		#endregion
	}
}
